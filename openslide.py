...
def getMag(s, params):
    ...
    if (mag == "NA" or strtobool(
        ...
        logging.warning(
            f"{s['filename']} - Unknown base magnification for file")
        s["warnings"].append(
            f"{s['filename']} - Unknown base magnification for file")
    else:
        mag = float(mag)
...
class BaseImage(dict):
    ...
    def __init__(self, fname, fname_outdir, params):
        ...
        self["mask_statistics"] = params.get(
        ...
        self["img_mask_use"] = np.ones(self.getImgThumb(
            self["image_work_size"]).shape[0:2], dtype=bool)
        self["img_mask_force"] = []
        ...
        if key not in self:
            ...
            if dim.replace(".", "0", 1).isdigit():  # check to see if dim is a number
                ...
                if dim < 1 and not dim.is_integer():  # specifying a downscale factor from base
                    ...
                    img = osh.read_region(
                        (0, 0), dim, osh.level_dimensions[dim])
                ...
                output = output[0:round(
                    dim_base[1] * 1 / down_factor), 0:round(dim_base[0] * 1 / down_factor), :]
                self[key] = output
            else:
                logging.error(
